...
Listado.prototype.buscarRestaurante = function(id) {
    ...
    var restauranteBuscado = this.restaurantes.find(restaurante => restaurante.id == id);

    if (restauranteBuscado != null) return restauranteBuscado;
    else return "No se ha encontrado ningún restaurant";
}


//Elimina los elementos repetidos de un arreglo.
function eliminarElementosRepetidos(elementos) {
    var arregloFiltrado = elementos.filter(function(elem, index, self) {
...
Listado.prototype.obtenerUbicaciones = function() {
    ...
    //Se agregan las ciudades al arreglo de ubicaciones.
    ubicaciones = this.restaurantes.map(function(restaurante) {
        return restaurante.ubicacion;
    })

    //Se retorna el arreglo de ubicaciones sin repetir y ordenadas.
    return eliminarElementosRepetidos(ubicaciones).sort();
}
...
Listado.prototype.obtenerRubros = function() {
    ...
    //Se agregan los rubros al arreglo de rubros.
    rubros = this.restaurantes.map(function(restaurante) {
        return restaurante.rubro;
    })

    //Se retorna el arreglo de rubros sin repetir y ordenados.
...
Listado.prototype.obtenerHorarios = function() {
    ...
    //Utilizamos map para recorrer los restaurantes y forEach para agregar los horarios al arreglo, incluyendo repetidos.
    this.restaurantes.map(function(restaurante) {
        return restaurante.horarios.forEach(function(horario) {
            horarios.push(horario);
        })
    })

    //Se retorna el arreglo horarios pero sin repetidos.
