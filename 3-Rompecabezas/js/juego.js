...
var instrucciones = ["Mueve los bloques hasta formar la imagen modelo.", "Para mover los bloques debes utilizar las flechas de tu teclado: ↑ ↓ → ← "];
...
function chequearSiGano() {
    ...
    for (i = 0; i < grilla.length; i++) {
        ...
        for (j = 0; j < grilla[i].length; j++) {
            ...
            if (grilla[i][j] != (3 * i + (j + 1))) return false;
        }
    }
    return true; //Si el ciclo for nunca se interrumpe es porque la grilla es la trivial.
}

//Funcion que reinicia el juego luego de ganar.
function volverAJugar() {
    mezclarPiezas(6);
    movimientos = []; //Reiniciamos los movimientos
    capturarTeclas();
}

/* Implementar alguna forma de mostrar un cartel que avise que ganaste el juego
    Me propuse hacer un cartel desde 0 utilizando lo que aprendí de DOM.
*/
function mostrarCartelGanador() {
    var cartel = document.createElement("div");
    var principal = document.getElementById("juego");

    cartel.id = "cartel_ganador"; //Se agrega un id para darle estilo.
    var mensaje_ganador = document.createElement("p");
    mensaje_ganador.innerText = "FELICITACIONES! \nGanaste en : " + movimientos.length + " movimientos.";
    cartel.appendChild(mensaje_ganador);

    var boton = document.createElement("button");
    boton.innerText = "VOLVER A JUGAR";
    boton.id = "boton_ganador";
    boton.addEventListener("click", function() {
        cartel.style.display = "none"; //Ocultamos el cartel ganador al presionar "Volver a jugar".
        volverAJugar();
    });

    cartel.appendChild(boton);

    principal.appendChild(cartel);
}
...
function actualizarPosicionVacia(nuevaFila, nuevaColumna) {
    for (i = 0; i < grilla.length; i++) {
        ...
        for (j = 0; j < grilla[i].length; j++) {
            ...
            if (grilla[i][j] == 9) {
                intercambiarPosicionesGrilla(i, j, nuevaFila, nuevaColumna);
                filaVacia = nuevaFila;
...
function mezclarPiezas(veces) {
    ...
    movimientos = []; //Reseteo la cantidad de movimientos para que funcione el contador.
}
    ...
    document.body.onkeydown = (function(evento) {
        if (evento.which === codigosDireccion.ABAJO ||
            ...
            if (gano) {
                ...
                setTimeout(function() {
                    document.body.onkeydown = null; //Detenemos la captura de teclas.
